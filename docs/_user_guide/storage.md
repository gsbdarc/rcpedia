...
The GSB offers nearly 1 PB of high-performance storage via the Yen servers, accessible at the ```/zfs``` path. Currently, all Yen user directories (``` ~/```) reside on ZFS. In addition to home directories, Stanford GSB faculty—and in some cases, students (see below)—can request additional project space on ZFS.
...
Every user on the Yens has a home directory. Home directories on the Yen servers are also stored on our storage system. The home directory has a quota of 50 GB.
...
!!! Note
    To create a new faculty or student project space, please complete the project space request form [here](http://darc.stanford.edu/Yenstorage/){:target="_blank"}. If you would like to discuss specific storage solutions for your project, Please email the [DARC](mailto:gsb_darcresearch@stanford.edu) team to discuss further.

### Scratch Space

There is a large ZFS based scratch space, accessible from any Yen, at ```/scratch/shared```, which is 100 TB in size. Reads and writes to this space are slower than to local disk.
